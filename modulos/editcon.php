...
$strsql = "SELECT idcategoria, nombre_categoria, descripcion FROM categorias WHERE idcategoria=?";
if($stmt = $mysqli->prepare($strsql)){
    ...
    if (isset($_POST['edit'])){
        ...
        strlen($_POST['descripcion']) >= 1)
        {
            ...
            $strsql ="UPDATE categorias SET nombre_categoria = '$nombre_categoria', descripcion = '$descripcion' WHERE idcategoria = '$idcategoria'";
            $resultado=mysqli_query($mysqli,$strsql);
            if ($resultado) {
            ?>
                <h3>Consola actualizada de Manera Exitosa</h3>
                <?php
                mysqli_close($mysqli);
            } else {
                ?>
                <h2>Error al actualizar la consola.</h2>
                <?php
            }
        }else {
            ?>
